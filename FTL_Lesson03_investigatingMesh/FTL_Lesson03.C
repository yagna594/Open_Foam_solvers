...
int main(int argc, char *argv[])
{
   ...
   {
        //std::string pname1= mesh.boundaryMesh()[patchI].name();
        //Info << pname1 <<nl;
        //word patchName("pname");
        label patchID(0);
        scalar sufce_Area = 0;  
        //Info << patchName <<nl;
   ...
   /* for (label patchI = 0; patchI < mesh.boundaryMesh().size(); patchI++)
    {
        scalar sufce_Area = 0;
        std::string patchName = mesh.boundaryMesh()[patchI].name();
        label patchID = mesh.boundaryMesh().findPatchID(patchName);
        forAll(mesh.boundary()[patchID].Sf(), faceI)
        {
            sufce_Area += mag(mesh.boundary()[patchID].Sf()[faceI]);
        }
        Info << "Surface Area of " << patchName << " = " << sufce_Area << endl;
    }
    Info << endl;*/
/*  forAll(mesh.boundaryMesh(),patchI)
